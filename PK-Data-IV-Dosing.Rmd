...
1. `CID`: unique identifier for each individual in the dataset; with "C" in the first column name, a NONMEM control file will ignore all column names so that users can redefine them using the required data items name.
1. `time`: time of either dosing or concentration measurement.
1. `amt`: dose amount; `NA` for concentration events.
1. `conc`: drug blood concentration value, which is DV (dependentt variable) as NONMEM data item; `NA` for a dose event.
1. rate: rate of infusion of drug; rate = 0 if a bolus is given.
