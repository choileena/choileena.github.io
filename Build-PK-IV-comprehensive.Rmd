...
The 4 types of raw datasets must go through a pre-processing stage which creates new ID variables and datasets that can be used by the data processing modules. There are three main pre-processing steps:
...
* **`readTransform()`**:  (NOTE describe briefly)
...
* **Concentration raw data**
    ...
    + (NOTE: explain what if concentration file comes with only one file with all info?)
...
* **Dosing raw data**
    ...
    + (NOTE: explain what if there is only one dosing file?)
...
* **Laboratory raw data**
    ...
    + (NOTE: explain what if there is only one file or more than 2 files?)
...
* **Options and parameters**: Before running the processing modules, it is necessary to define several options and parameters. 
    ...
    + We can create a `drugname` stub.
    + We can define the lower limit of quantification (LLOQ) for the drug of interest.
...
* This module processes structured medication data. Only Part I which handles IV dose data is described here. For processing e-prescription data (which is another structured medication data), see [*Pro-Med-Str* - Part II](Pro-Med-Str-Part2.html).
...
* Both dose data from the *Pro-Med-Str1* module and concentration data from the *Pro-DrugLevel* module are required. 
* Demographic data from the *Pro-Demographic* module and laboratory data from the *Pro-Laboratory* module are optional. 
* The module is semi-interactive -- it generates several files to check potential data errors, and get feedback from an investigator. If corrected information (‘fix’ files) are provided, the module should be re-run to incorporate the corrections.
* **run_Build_PK_IVs()** is the function to build PK data with IV dosing data.
