...
* The output from the *Pro-Med-Str* - Part I module is a `data.frame` with the following columns:  
  ...
  - `maxint`: infusion recording interval (e.g., default: maxint = 15 min for MAR data; maxint = 60 min for flow data). Depending institutions, this output variable should be used with caution when running `run_Build_PK_IV`. In Vanderbilt University Medical Center after *Epic* EHR system was implemented, there are only MAR data (no flow sheet data) which have infusion data recorded with any time intervals and hence `maxint` should not be set for 'gap' in `dose.columns` argument of `run_Build_PK_IV` function.
