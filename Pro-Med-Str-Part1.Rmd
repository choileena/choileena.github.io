...
* The output from the *Pro-Med-Str* - Part I module is a `data.frame` with the following columns:  
  ...
  - `maxint`: infusion recording interval (e.g., default: maxint = 15 min for MAR data; maxint = 60 min for flow data). Depending institutions, this output variable should be used with caution when running `run_Build_PK_IV`. For example of Vanderbilt University Medical Center, after *Epic* EHR system was implemented, only MAR data are collected without flow sheet data, which have infusion data recorded with any time intervals, and hence `maxint` should not be set for 'gap' in `dose.columns` argument of `run_Build_PK_IV` function. See [2. EHR Vignette for Structured Data](https://cran.r-project.org/web/packages/EHR/vignettes/ehr_vignette_02_str.html) of `EHR` package for details. 
