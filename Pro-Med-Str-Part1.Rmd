...
idcols <-  list(c('subject_id', 'subject_uid'), # id vars in flow.in
...
# Input raw dose data
* IV dose data can be in various forms, which may need to be pre-processed. As IV dose data can be obtained from different data sources when using electronic health records (EHRs), we illustrate this module using example datasets that come from two sources:
...
* Note that if dose data come from a single data file, only this data file needs to be read in.

The cleaned, structured data saved as `R` objects for use with `run_MedStrI()` module are shown below.

```{r cleaned dosing data}
# flow dosing data
head(flow.cln)
...
Below we show how we would run `run_MedStrI()` using the cleaned dose datasets (i.e., the Flow data and the MAR data) as input.
